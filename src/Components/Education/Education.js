export const Education = () => {
    return (
        <section className="p-1">
            <h1 className="text-2xl font-semibold">Education</h1>
            <article className="mb-2">
                <h1 className="text-xl font-semibold">School</h1>
                <p>May 2015</p>
                <h3 className="text-l font-semibold">High School Diploma, North Gate High School</h3>
                <p>Graduted on time with class of 2015</p>
            </article>

            <article className="mt-2">
                <h1 className="text-xl font-semibold">Certificate</h1>
                <p>November 2019</p>
                <h3 className="text-l font-semibold">Full Stack Web Development, Georgia Tech</h3>
                <p>Incredibly in-depth and fast paced bootcamp. Students were taught the latest technology behind creating websites from the ground up using both frontend and backend technologies.</p>
            </article>
        </section>
    )
}